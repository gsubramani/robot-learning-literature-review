...
where $`\{\beta_i\}`$ is a mixing schedule satisfying $`\beta_i \to 0`$ as $`i \to \infty`$ (e.g., $`\beta_i = p^{i-1}`$ for $`p \in (0,1)`$, or $`\beta_1 = 1, \beta_{i\gt 1} = 0`$ for the parameter-free variant). At early iterations, the expert provides most of the control, ensuring safe data collection; as $`\hat{\pi}`$ matures, the learner increasingly controls the trajectory and the expert labels states near the learner's distribution.
