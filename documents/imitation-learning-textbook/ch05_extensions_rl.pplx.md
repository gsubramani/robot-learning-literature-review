...
where $`\bar{f}(s) = \frac{1}{N}\sum_i f_i(s)`$. The policy acts autonomously if $`\sigma^2_\mathrm{ensemble}(s) \lt \chi`$ (doubt threshold); otherwise, the expert is queried. This dual rule—low discrepancy *and* low doubt—constrains the probability of failure more tightly than single-threshold approaches.
